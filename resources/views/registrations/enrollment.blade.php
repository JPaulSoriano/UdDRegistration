...
<div class="col-lg-12">
    ...
    <table class="table table-striped table-borderless table-responsive table-sm" style="font-size: 12px" id="registration">
        ...
        <tbody>
        @foreach ($enrollment as $registration)
	    <tr>
            ...
            <button type="button" class="btn btn-sm btn-block btn-primary" data-toggle="modal" data-target="#modal-{{ $registration->id }}">
            ...
            <td id="or">{{ $registration->or_no }}</td>
                ...
                @role('Sao|Super Admin')
                    ...
                    <form action="{{ route('registrations.unadmit', $registration) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-block btn-danger">Unadmit</button>
                    </form>
                    @else
                    <!-- <a href="{{ route('registrations.admit', $registration) }}"
                        class="btn btn-sm btn-block btn-primary">Admit</a> -->
                        <button class="btn btn-sm btn-block btn-primary admit" data-id="{{ $registration->id }}">Admit</button>
                    @endif
                        @if($registration->status_enrollment == 0)
                            <a href="#" class="btn btn-secondary btn-sm btn-block admit" data-id="{{ $registration->id }}">Temp Stud No</a>
                        @endif
                    </td>
                @endrole
                ...
                @role('Cashier|Super Admin|Sao')
                    <td>
                        ...
                        <form action="{{ route('registrations.unverify', $registration) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-block btn-danger">Unverify</button>
                        </form>
                        @else
                        <!-- <a href="{{ route('registrations.verify', $registration) }}"
                            class="btn btn-sm btn-block btn-primary">Verify</a> -->
                        <button type="button" class="btn btn-sm btn-block btn-primary orno" data-id="{{ $registration->id }}">Verify</button>
                        @endif
                        @if($registration->or_no)
                        <a href="#" class="btn btn-secondary btn-sm btn-block orno" data-id="{{ $registration->id }}">OR No</a>
                        @endif
                ...
                @role('Dean|Super Admin|Sao|Acad')
                    <td>
                      ...
                      <form action="{{ route('registrations.unenroll', $registration) }}" method="POST">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn btn-sm btn-block btn-danger">Unenroll</button>
                      </form>
                      @else
                      <!-- <a href="{{ route('registrations.enroll', $registration) }}"
                          class="btn btn-sm btn-block btn-primary">Enroll</a> -->
                      <button class="btn btn-sm btn-block btn-primary enrol" data-id="{{ $registration->id }}">Enroll</button>
                      @endif
                      <a href="#" class="btn btn-secondary btn-sm btn-block enrol" data-id="{{ $registration->id }}">Stud No</a>
                    </td>
                @endrole
	    </tr>
        @endforeach
